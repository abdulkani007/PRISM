...
export default function GptChatInterface({ currentUser, onSignOut, onBackToHome }) {
  ...
  return (
    ...
    >
      ...
      {isDragging && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/85 backdrop-blur-md border-2 border-dashed border-amber-500/80 pointer-events-none transition-all">
          <div className="p-6 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400 mb-4 animate-bounce">
            <UploadCloud className="w-12 h-12" />
          </div>
          <h3 className="text-xl font-medium tracking-wide text-amber-300">
      ...
      {imageModalUrl && (
        ...
        >
          <div className="relative max-w-4xl max-h-[90vh] bg-neutral-900 rounded-2xl border border-white/10 p-2 overflow-hidden shadow-2xl">
            <button
              onClick={() => setImageModalUrl(null)}
              className="absolute top-4 right-4 p-2 rounded-full bg-black/70 hover:bg-black text-white/80 hover:text-white transition-colors cursor-target"
            >
              <X className="w-5 h-5" />
            </button>
      ...
      {/* LEFT SIDEBAR */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex flex-col w-72 bg-[#0a0a0c] border-r border-white/10 transition-transform duration-300 ease-in-out md:static ${
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-white/10">
          <div className="flex items-center space-x-3 cursor-pointer" onClick={onBackToHome}>
            <img src={logoImg} alt="PRISM" className="w-7 h-7 object-contain" />
            <div>
              <div className="font-semibold text-sm tracking-wider text-white flex items-center gap-1.5">
                PRISM <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-400 border border-amber-500/30 font-mono">AI</span>
              </div>
        ...
        <div className="p-3">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-amber-500/40 text-sm font-medium text-white transition-all shadow-sm cursor-target group"
          >
        ...
        <div className="p-3 border-t border-white/10 bg-[#08080a] space-y-2">
          <div className="flex items-center gap-3 p-2 rounded-xl bg-white/5 border border-white/5">
            <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-amber-500 to-orange-500 flex items-center justify-center font-bold text-xs text-black shrink-0">
          ...
          <div className="flex items-center gap-1.5 pt-1">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-lg text-xs text-neutral-400 hover:text-white hover:bg-white/5 border border-transparent hover:border-white/10 transition-all cursor-target"
            ...
            <button
              onClick={onSignOut}
              className="flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-lg text-xs text-red-400 hover:text-red-300 hover:bg-red-500/10 border border-transparent hover:border-red-500/20 transition-all cursor-target"
              title="Sign Out"
            >
      ...
      <main className="flex-1 flex flex-col h-full bg-[#050507] overflow-hidden relative">
        {/* Top Header */}
        <header className="flex items-center justify-between px-6 py-3.5 border-b border-white/10 bg-black/40 backdrop-blur-md z-10">
          ...
          <div className="flex items-center gap-2">
            {messages.length > 0 && (
              <button
                onClick={handleNewChat}
                className="flex items-center gap-1.5 text-xs text-neutral-400 hover:text-white px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 transition-colors cursor-target"
              >
          ...
          {messages.length === 0 ? (
            ...
            <div className="max-w-3xl mx-auto my-auto flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
              <div className="relative mb-6">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-amber-500/20 to-orange-500/20 border border-amber-500/40 flex items-center justify-center shadow-[0_0_50px_rgba(245,158,11,0.25)]">
              ...
              <p className="text-sm text-neutral-400 max-w-xl mb-8 leading-relaxed">
                ...
                <span className="text-amber-400 font-medium">drag & drop a photo</span> to run 68-point facial biometric
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full text-left">
                {SAMPLE_SUGGESTIONS.map((item, idx) => (
                  <button
                    ...
                    className="p-4 rounded-2xl bg-neutral-900/60 hover:bg-neutral-900 border border-white/10 hover:border-amber-500/40 text-left transition-all hover:scale-[1.01] group cursor-target"
                  >
            ...
            <div className="max-w-3xl mx-auto space-y-6">
                ...
                <div key={msg.id} className="space-y-2">
                  ...
                  {msg.role === 'user' ? (
                    <div className="flex justify-end items-start gap-3">
                      <div className="max-w-[85%] rounded-3xl rounded-tr-sm bg-neutral-800/90 border border-white/10 p-4 text-white shadow-lg space-y-3">
                        ...
                        {msg.image && (
                          <div className="relative group rounded-xl overflow-hidden border border-white/20 bg-black/40">
                            ...
                            />
                            <div className="absolute top-2 left-2 px-2 py-1 rounded-md bg-black/70 backdrop-blur-md text-[10px] font-mono text-amber-300 border border-amber-500/30 flex items-center gap-1.5">
                        ...
                        <div className="text-[10px] text-neutral-400 text-right font-mono">{msg.timestamp}</div>
                      </div>
                      <div className="w-8 h-8 rounded-full bg-neutral-800 border border-white/10 flex items-center justify-center text-xs font-semibold shrink-0">
                        {currentUser?.displayName ? currentUser.displayName[0].toUpperCase() : 'U'}
                      </div>
                    </div>
                  ) : (
                    ...
                    <div className="flex justify-start items-start gap-3">
                      <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-amber-500 to-orange-500 flex items-center justify-center text-black font-bold shrink-0 shadow-[0_0_15px_rgba(245,158,11,0.3)]">
                        <Sparkles className="w-4 h-4 text-black" />
                      </div>

                      <div className="max-w-[90%] w-full rounded-3xl rounded-tl-sm bg-[#0d0d11] border border-white/10 p-5 text-neutral-100 shadow-xl space-y-5">
                        {/* Header & Title */}
                        {msg.data?.title && (
                          <div className="flex items-center justify-between border-b border-white/10 pb-3">
                            ...
                            <button
                              ...
                              className="flex items-center gap-1 text-[11px] text-neutral-400 hover:text-white px-2 py-1 rounded bg-white/5 hover:bg-white/10 transition-colors cursor-target"
                        ...
                        {/* BIOMETRICS ANALYSIS CARD (If photo was analyzed) */}
                        {msg.data?.biometrics && (
                          <div className="p-4 rounded-2xl bg-neutral-900/80 border border-amber-500/20 space-y-3">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px]">
                              ...
                              <div className="p-2 rounded-xl bg-black/40 border border-white/5">
                                ...
                                <div className="text-emerald-400 font-mono font-medium mt-0.5">{msg.data.biometrics.tamperRisk}</div>
                              </div>
                              <div className="p-2 rounded-xl bg-black/40 border border-white/5">
                              ...
                              <div className="p-2 rounded-xl bg-black/40 border border-white/5">
                                <div className="text-amber-300 font-mono truncate mt-0.5">{msg.data.biometrics.perceptualHash}</div>
                              </div>
                            </div>
                          </div>
                        )}
                        ...
                        {msg.data?.matchedCandidates?.map((cand, idx) => (
                          <div key={idx} className="p-4 rounded-2xl bg-neutral-900/60 border border-white/10 space-y-3">
                            <div className="flex items-center justify-between">
                              <div>
                              ...
                              <div className="text-right">
                                <span className="text-xs font-mono text-amber-400 font-bold">{cand.handle}</span>
                                <div className="text-[10px] text-emerald-400 mt-0.5">{cand.status}</div>
                              </div>
                            </div>
                            ...
                            <div className="space-y-1.5 pt-1">
                              ...
                              {cand.sources.map((src, sIdx) => (
                                <div
                                  key={sIdx}
                                  className="flex items-center justify-between py-1.5 px-2.5 rounded-lg bg-black/30 border border-white/5 text-xs"
                                >
                                  <div className="flex items-center gap-2">
                        ...
                        {msg.data?.candidate && (
                          <div className="p-4 rounded-2xl bg-neutral-900/60 border border-white/10 space-y-3">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="text-right">
                                <span className="text-xs font-mono text-amber-400 font-bold">{msg.data.candidate.handle}</span>
                                <div className="text-[10px] text-emerald-400 font-mono">{msg.data.candidate.confidence}% Confidence</div>
                              </div>
                            </div>
                            ...
                            {msg.data.evidence && (
                              <div className="space-y-1.5 pt-1">
                                ...
                                {msg.data.evidence.map((ev, eIdx) => (
                                  <div
                                    key={eIdx}
                                    className="p-2 rounded-lg bg-black/30 border border-white/5 text-xs flex items-start gap-2"
                                  >
                        ...
                        {msg.data?.sha256 && (
                          <div className="p-2.5 rounded-xl bg-black/50 border border-white/5 flex items-center justify-between text-[10px] font-mono text-neutral-400">
                            ...
                            <button
                              ...
                              className="text-neutral-400 hover:text-white px-2 py-0.5 rounded bg-white/5 hover:bg-white/10 transition-colors shrink-0 flex items-center gap-1 cursor-target"
                            >
                            ...
                            <div className="flex flex-wrap gap-2">
                              {msg.data.followUpQuestions.map((q, qIdx) => (
                                <button
                                  ...
                                  className="text-xs text-left py-1.5 px-3 rounded-full bg-white/5 hover:bg-amber-500/15 text-neutral-300 hover:text-amber-300 border border-white/10 hover:border-amber-500/30 transition-all cursor-target"
                                >
                                  {q}
                                </button>
                              ))}
              ...
              {isInvestigating && (
                <div className="flex justify-start items-start gap-3 animate-pulse">
                  ...
                  <div className="rounded-3xl rounded-tl-sm bg-[#0d0d11] border border-amber-500/30 p-4 text-xs text-neutral-300 shadow-xl space-y-2 min-w-[280px]">
        ...
        {/* BOTTOM PROMPT INPUT BAR (STYLED EXACTLY LIKE THE USER'S REFERENCE IMAGE) */}
        <div className="w-full px-4 md:px-8 pb-6 pt-2 bg-gradient-to-t from-black via-black/90 to-transparent">
          <div className="max-w-3xl mx-auto flex flex-col items-center">
            ...
            {attachedImage && (
              <div className="w-full mb-3 flex items-center gap-3 p-2.5 rounded-2xl bg-neutral-900/90 border border-amber-500/40 shadow-[0_0_20px_rgba(245,158,11,0.2)]">
            ...
            {/* GLOWING CAPSULE / PILL INPUT BAR */}
            <div className="relative w-full group">
              {/* AMBIENT WARM AMBER / GOLDEN HALO GLOW (matches user reference image) */}
              <div className="absolute -inset-1 rounded-full bg-gradient-to-r from-amber-500/25 via-amber-600/30 to-orange-500/25 blur-xl opacity-75 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />

              {/* MAIN PILL CONTAINER */}
              <div className="relative flex items-center justify-between w-full h-14 md:h-16 px-3 md:px-4 rounded-full bg-[#0d0f17] border border-amber-500/35 shadow-[0_0_35px_rgba(245,158,11,0.25)] backdrop-blur-xl">
                {/* LEFT SPARKLE BUTTON & PHOTO ATTACHMENT */}
                <div className="flex items-center gap-1.5 shrink-0 pl-1">
                  {/* Glowing Sparkle Icon Button (from reference) */}
                  <div
                    className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 flex items-center justify-center text-amber-400 transition-all cursor-target shadow-inner"
                  ...
                  <button
                    ...
                    className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-white/5 hover:bg-white/10 text-neutral-400 hover:text-white flex items-center justify-center transition-colors cursor-target"
                ...
                <div className="flex-1 flex flex-col justify-center px-3 md:px-4 min-w-0">
                  <input
                    ...
                    className="w-full bg-transparent text-white text-xs md:text-sm placeholder-neutral-400 focus:outline-none truncate"
                  />
                ...
                {/* RIGHT CIRCULAR SUBMIT BUTTON WITH GRADIENT ARROW (matches user reference image) */}
                <button
                  ...
                  className={`w-9 h-9 md:w-11 md:h-11 rounded-full flex items-center justify-center transition-all cursor-target shrink-0 shadow-lg ${
                    isInvestigating || (!inputPrompt.trim() && !attachedImage)
                      ...
                      : 'bg-gradient-to-tr from-amber-600 via-orange-500 to-amber-400 text-black hover:scale-105 hover:shadow-[0_0_20px_rgba(245,158,11,0.5)] active:scale-95'
                  }`}
                  title="Run Investigation"
                >
                  {isInvestigating ? (
                    <RefreshCw className="w-4 h-4 md:w-5 md:h-5 animate-spin text-white" />
                  ) : (
